...
from f1 import production_list, nt_list as ntl, t_list as tl
nt_list, t_list=[], []
    ...
    while True:
        ...
        for i in items: 
            ...
            if i.index('.')+1<len(i)-1:
                lastr=list(f1.compute_first(i[i.index('.')+2])-set(chr(1013)))
...
def main():
    ...
    print("---------------First AND Follow Of Varibles---------------")
    for nt in ntl:
        f1.compute_first(nt)
        f1.compute_follow(nt)
        print(nt)
        print("\tFirst:\t", f1.get_first(nt))
        print("\tFollow:\t", f1.get_follow(nt), "\n")  
    ...
    print('_____________________________________________________________________')
    print("Input the string for parsing:")
    Input=input()+'$'
    try:
        stack=['0']
        a=list(table.items())
